/**
 * @file transfer.c
 * @brief Functions for sending and receiving files.
 *
 * This file implements the core logic for transferring files over a TCP socket.
 * It handles both sending and receiving files, including metadata like filename,
 * size, and permissions.
 */

#include "transfer.h"
#include "progress_bar.h"
#include <arpa/inet.h>      // For inet_ntoa()
#include <fcntl.h>          // For open(), O_RDONLY, O_CREAT, O_WRONLY, O_TRUNC
#include <netdb.h>          // For getaddrinfo(), freeaddrinfo(), struct addrinfo
#include <stdio.h>          // For printf(), fprintf(), perror()
#include <stdlib.h>         // For exit(), EXIT_FAILURE
#include <string.h>         // For strncpy(), memset()
#include <sys/socket.h>     // For socket(), bind(), listen(), accept(), send(), recv(), connect()
#include <sys/stat.h>       // For fstat(), struct stat
#include <unistd.h>         // For close(), read(), write()

// Use platform-specific includes for sendfile for efficiency.
#if defined(__APPLE__)
#include <sys/uio.h>      // For sendfile()
#elif defined(__linux__)
#include <sys/sendfile.h> // For sendfile()
#endif

#define PORT "7979"
#define BUFFER_SIZE (1024 * 512) // 512KB buffer for file transfer

// ------------------------- SHARE FUNCTION -------------------------

/**
 * @brief Sends a file to a connected receiver.
 *
 * This function opens the specified file, sends its metadata (name, size, mode),
 * and then the file content over a TCP socket.
 *
 * @param filepath The path to the file to be sent.
 * @return 0 on success, -1 on error.
 */
int transferSend(const char *filepath) {
  if (!filepath) {
    fprintf(stderr, "No file path provided.\n");
    return -1;
  }

  printf("%s[INFO]%s Preparing to share file: %s\n", COLOR_BLUE, COLOR_END,
         filepath);

  // Open the file for reading.
  int fd = open(filepath, O_RDONLY);
  if (fd < 0) {
    perror("open");
    return -1;
  }

  // Get file stats to retrieve size and mode.
  struct stat st;
  if (fstat(fd, &st) < 0) {
    perror("fstat");
    close(fd);
    return -1;
  }

  // Populate the metadata struct.
  FileMetaData metadata = {0};
  strncpy(metadata.filename, filepath, sizeof(metadata.filename) - 1);
  metadata.size = st.st_size;
  metadata.mode = st.st_mode;

  // Set up the TCP server to listen for a connection.
  struct addrinfo hints = {0}, *res;
  hints.ai_family = AF_INET;       // Use IPv4
  hints.ai_socktype = SOCK_STREAM; // Use TCP
  hints.ai_flags = AI_PASSIVE;     // Listen on any available network interface

  if (getaddrinfo(NULL, PORT, &hints, &res) != 0) {
    perror("getaddrinfo");
    close(fd);
    return -1;
  }

  // Create a socket.
  int sockfd = socket(res->ai_family, res->ai_socktype, res->ai_protocol);
  if (sockfd < 0) {
    perror("socket");
    freeaddrinfo(res);
    close(fd);
    return -1;
  }

  // Bind the socket to the specified port.
  if (bind(sockfd, res->ai_addr, res->ai_addrlen) != 0) {
    perror("bind");
    freeaddrinfo(res);
    close(fd);
    return -1;
  }

  // Listen for incoming connections.
  if (listen(sockfd, 1) != 0) {
    perror("listen");
    freeaddrinfo(res);
    close(fd);
    return -1;
  }

  printf("%s[WAIT]%s Waiting for receiver to connect on port %s...\n",
         COLOR_YELLOW, COLOR_END, PORT);

  // Accept a connection from a client.
  struct sockaddr_storage client_addr;
  socklen_t client_len = sizeof(client_addr);
  int clientfd = accept(sockfd, (struct sockaddr *)&client_addr, &client_len);
  if (clientfd < 0) {
    perror("accept");
    freeaddrinfo(res);
    close(fd);
    close(sockfd);
    return -1;
  }

  // Send the file metadata to the receiver.
  if (send(clientfd, &metadata, sizeof(metadata), 0) != sizeof(metadata)) {
    perror("send metadata");
    close(fd);
    close(clientfd);
    close(sockfd);
    freeaddrinfo(res);
    return -1;
  }
  printf("%s[SENDING]%s Sent [%s]'s metadata\n", COLOR_BLUE, COLOR_END,
         metadata.filename);

  printf("%s[SENDING]%s File: %s (%lld bytes)\n", COLOR_BLUE, COLOR_END,
         metadata.filename, (long long)metadata.size);

  // Use sendfile for efficient file transfer on supported platforms.
#if defined(__APPLE__) || defined(__linux__)
  off_t offset = 0;
  ssize_t sent_bytes;
  while (offset < metadata.size) {
    #if defined(__APPLE__)
      sent_bytes = sendfile(fd, clientfd, offset, (off_t *)&metadata.size, NULL, 0);
    #elif defined(__linux__)
      sent_bytes = sendfile(clientfd, fd, &offset, metadata.size - offset);
    #endif

    if (sent_bytes <= 0) {
        if (sent_bytes < 0) perror("sendfile");
        break;
    }
    offset += sent_bytes;
    progressBarUpdate(offset, metadata.size);
  }
#else
  // Fallback to a buffer-based send for other systems.
  char buffer[BUFFER_SIZE];
  ssize_t bytes_read;
  off_t sent_total = 0;

  while ((bytes_read = read(fd, buffer, sizeof(buffer))) > 0) {
    ssize_t total_sent = 0;
    while (total_sent < bytes_read) {
      ssize_t sent = send(clientfd, buffer + total_sent, bytes_read - total_sent, 0);
      if (sent < 0) {
        perror("send (fallback)");
        break;
      }
      total_sent += sent;
      sent_total += sent;
      progressBarUpdate(sent_total, metadata.size);
    }
  }
#endif

  printf("\n%s[DONE]%s Sent file successfully!\n", COLOR_GREEN, COLOR_END);

  // Clean up resources.
  close(fd);
  close(clientfd);
  close(sockfd);
  freeaddrinfo(res);
  return 0;
}

// ------------------------- RECEIVE FUNCTION -------------------------

/**
 * @brief Receives a file from a sender.
 *
 * This function connects to the sender's TCP server, receives the file metadata,
 * and then the file content, saving it to a new file.
 *
 * @param server_addr The IP address of the sender.
 * @return 0 on success, -1 on error.
 */
int transferReceive(const char *server_addr) {
  // Set up hints for getaddrinfo.
  struct addrinfo hints = {0}, *res;
  hints.ai_family = AF_INET;       // Use IPv4
  hints.ai_socktype = SOCK_STREAM; // Use TCP

  // Resolve the server address.
  if (getaddrinfo(server_addr, PORT, &hints, &res) != 0) {
    perror("getaddrinfo");
    return -1;
  }

  // Create a socket.
  int sockfd = socket(res->ai_family, res->ai_socktype, res->ai_protocol);
  if (sockfd < 0) {
    perror("socket");
    freeaddrinfo(res);
    return -1;
  }

  printf("%s[INFO]%s Connecting to %s:%s ...\n", COLOR_BLUE, COLOR_END,
         server_addr, PORT);

  // Connect to the sender.
  if (connect(sockfd, res->ai_addr, res->ai_addrlen) != 0) {
    perror("connect");
    freeaddrinfo(res);
    close(sockfd);
    return -1;
  }

  // Receive the file metadata.
  FileMetaData metadata = {0};
  if (recv(sockfd, &metadata, sizeof(metadata), 0) != sizeof(metadata)) {
    perror("recv metadata");
    freeaddrinfo(res);
    close(sockfd);
    return -1;
  }
  printf("%s[INFO]%s Received: %s's metadata\n", COLOR_BLUE, COLOR_END,
         metadata.filename);

  printf("%s[INFO]%s Receiving: %s (%lld bytes)\n", COLOR_BLUE, COLOR_END,
         metadata.filename, (long long)metadata.size);

  // Create and open the file for writing.
  int fd = open(metadata.filename, O_CREAT | O_WRONLY | O_TRUNC, metadata.mode);
  if (fd < 0) {
    perror("open");
    freeaddrinfo(res);
    close(sockfd);
    return -1;
  }

  // Receive the file content and write it to the file.
  off_t received = 0;
  char buffer[BUFFER_SIZE];

  while (received < metadata.size) {
    ssize_t bytes = recv(sockfd, buffer, sizeof(buffer), 0);
    if (bytes <= 0) {
      if (bytes < 0) perror("recv");
      break;
    }
    write(fd, buffer, bytes);
    received += bytes;
    progressBarUpdate(received, metadata.size);
  }
  printf("\n");

  printf("%s[DONE]%s Received %s (%lld bytes)\n", COLOR_GREEN, COLOR_END,
         metadata.filename, (long long)metadata.size);

  // Clean up resources.
  close(fd);
  close(sockfd);
  freeaddrinfo(res);
  return 0;
}
